...
class RecordAbsClass():
    def __init__(self, main_window, login_name, login, password, tags, main_key):
    ...
    def result_forms_dict(self, edit_section=False):
        ...
        if self.is_correct_forms(edit_section):
            encrypted_password, urandom = cypher_func.text_encryptor(self.main_key, self.password)
            encrypted_login = cypher_func.text_encryptor(self.main_key, self.login, urandom, False)
