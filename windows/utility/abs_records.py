...
class RecordAbsClass():
    ...
    def result_forms_dict(self, edit_section=False):
        ...
        if self.is_correct_forms(edit_section):
            encrypted_password, urandom = ciphers.text_encryptor(self.user_info.user_key, self.password)
            encrypted_login = ciphers.text_encryptor(self.user_info.user_key, self.login, urandom, False)
